...
const state=load();function save(){try{localStorage.setItem(REGIONS_KEY,JSON.stringify(state))}catch(e){}}
...
function canBuild(action){sync();const d=defs.find(x=>x.action===action);if(!d)return true;const s=state[d.id],cap=s.level*d.capacity;if(s.used<cap)return true;alert('المنطقة ممتلئة. وسّع '+d.name+' أولًا.');return false}
function render(){sync();const p=document.getElementById('regions');if(!p)return;p.innerHTML=defs.map(d=>{const s=state[d.id],cap=s.level*d.capacity,full=s.used>=cap;return `<div class="region" style="border-color:${d.color}"><div><b>${d.name}</b><small>${d.desc}</small></div><span>المساحة ${s.used}/${cap}</span><button type="button" data-expand-region="${d.id}">توسيع +1<br><small>${d.cost} 💰</small></button></div>`}).join('');document.querySelectorAll('[data-action]').forEach(b=>{const a=b.dataset.action;if(['farm','city','hospital','solar','research','explore'].includes(a))b.disabled=!canBuild(a)})}
function expand(id){const d=defs.find(x=>x.id===id),s=state[id],T=window.TerraNova,g=T&&T.state;if(!d||!s||!g)return;if(g.credits<d.cost){alert('لا توجد أموال كافية لتوسيع المنطقة');return}g.credits-=d.cost;s.level++;save();if(T&&T.render)T.render();render()}
document.addEventListener('click',e=>{const b=e.target.closest&&e.target.closest('[data-expand-region]');if(b){e.preventDefault();expand(b.dataset.expandRegion)}});window.addEventListener('load',()=>{render();setInterval(render,1000)});window.TerraNovaRegions={defs,state,render,expand,canBuild};
})();
(function(){'use strict';
if(window.__TERRANOVA_EVENTS_RUNNING)return;
window.__TERRANOVA_EVENTS_RUNNING=true;
const EVENTS=[
...
function trigger(){if(!window.TerraNova)return;const s=window.TerraNova.state,e=pick(),detail=e.apply(s);if(Array.isArray(s.log)){s.log.unshift('🎲 '+e.name+' — '+detail);s.log=s.log.slice(0,50)}window.TerraNova.save();window.TerraNova.render()}
window.TerraNovaEvents={trigger,pick};
setTimeout(()=>{if(!window.__TERRANOVA_EVENTS_TIMER)window.__TERRANOVA_EVENTS_TIMER=setInterval(trigger,15000)},7000);
})();